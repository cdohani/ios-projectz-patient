...
class GetTwoFactorAuthService: GenericService, @unchecked Sendable {
...
extension GetTwoFactorAuthService {
    
    fileprivate func parseUserInformationFromJsonString (jsonString: String) -> TwoFactorAuthReponseModel{
        ...
        do {
            if let dictionary = try JSONSerialization.jsonObject(with: jsonString.data(using: String.Encoding(rawValue: String.Encoding.utf8.rawValue))!, options: .allowFragments) as? [String: Any] {
                ...
                if let dataDic = dictionary["data"] as? [String: Any] {
                    if let val = dataDic["phone_number"] as? String {
                        data.phoneNum = val
                    }
                    if let val = dataDic["is_verified"] as? Bool {
                        data.isVerifiedPhone = val
                    }
                    if let val = dataDic["mfa_enabled"] as? Bool {
                        data.isTwoFactorAuthEnabled = val
                    }
                }
