...
extension GetHealthCheckHistoryService {
    
    fileprivate func parseUserInformationFromJsonString (jsonString: String) -> HealthCheckReponseModel{
        
       var data = HealthCheckReponseModel()
        ...
        do {
            if let dictionary = try JSONSerialization.jsonObject(with: jsonString.data(using: String.Encoding(rawValue: String.Encoding.utf8.rawValue))!, options: .allowFragments) as? [String: Any] {
                ...
                if let historyDic = dictionary["data"] as? [String: Any] {
                    ...
                    if let visitDic = historyDic["health_care"] as? [[String: Any]] {
                        var list = HealthCheckModel()
                        for item in visitDic{
                            list = HealthCheckModel()
                            ...
                            if let val = item["last_modified_at"] as? String {
                                list.lastModifiedAt = val
                            }
                            if let val = item["is_have_records"] as? Bool {
                                list.isSelected = val
                            }
