...
class GetSelectedGenderService: GenericService, @unchecked Sendable {
    
    func getData(userID:Int,completion: @escaping CompletionBlock, failure: @escaping FailureBlock) {
        ...
        var urlComponents = URLComponents(string: Constants.URLs.selectedGenders)
...
extension GetSelectedGenderService {
    
    fileprivate func parseUserInformationFromJsonString (jsonString: String) -> SelectedGenderResponseModel{
        
       var data = SelectedGenderResponseModel()
        ...
        do {
            if let dictionary = try JSONSerialization.jsonObject(with: jsonString.data(using: String.Encoding(rawValue: String.Encoding.utf8.rawValue))!, options: .allowFragments) as? [String: Any] {
                ...
                if let dataDic = dictionary["data"] as? [String: Any] {
                    if let val = dataDic["extra_genders"] as? String {
                        data.selectedGender = val
                    }
                }
                //SessionID
            } else {
