...
class GetSupportCategoryService: GenericService, @unchecked Sendable {
    ...
    func getData(completion: @escaping CompletionBlock, failure: @escaping FailureBlock) {
        ...
        var urlComponents = URLComponents(string: Constants.URLs.getSupportCategory)
...
extension GetSupportCategoryService {
    
    fileprivate func parseUserInformationFromJsonString (jsonString: String) -> SupportCategoryResponseModel{
        
       var data = SupportCategoryResponseModel()
        ...
        do {
            if let dictionary = try JSONSerialization.jsonObject(with: jsonString.data(using: String.Encoding(rawValue: String.Encoding.utf8.rawValue))!, options: .allowFragments) as? [String: Any] {
                ...
                if let dataDic = dictionary["data"] as? [String: Any] {
                    if let locationDic = dataDic["categories"] as? [[String: Any]] {
                        var list = CategoryDataModel()
                        for item in locationDic {
                            ...
                            if let subCategory = item["subcategories"] as? [[String: Any]] {
                                var subCate = SubCategoryDataModel()
                                for item in subCategory {
                                    subCate = SubCategoryDataModel()
                                    ...
                                    if let val = item["category_id"] as? Int {
                                        subCate.categoryID = val
                                    }
                                    if let val = item["created_at"] as? String {
                                        subCate.createdAt = val
                                    }
                                    if let val = item["updated_at"] as? String {
                                        subCate.updatedAt = val
                                    }
                                    list.arrSubCategory.append(subCate)
                                }
                            }
                            data.arrCategory.append(list)
                        }
                    }
                }
                //SessionID
