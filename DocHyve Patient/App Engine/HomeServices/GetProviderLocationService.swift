...
class GetProviderLocationService: GenericService, @unchecked Sendable {
    ...
    func getData(providerID : Int,completion: @escaping CompletionBlock, failure: @escaping FailureBlock) {
        ...
        var endPoint = String(format: Constants.URLs.getProviderAddress, providerID)
...
extension GetProviderLocationService {
    
    fileprivate func parseUserInformationFromJsonString (jsonString: String) -> LocationResponseModel{
        
       var data = LocationResponseModel()
            ...
            if let dictionary = try JSONSerialization.jsonObject(with: jsonString.data(using: String.Encoding(rawValue: String.Encoding.utf8.rawValue))!, options: .allowFragments) as? [String: Any] {
                ...
                if let dataDic = dictionary["data"] as? [String: Any] {
                    if let locationDic = dataDic["addresses"] as? [[String: Any]] {
                        var list = LocationDataModel()
                        for item in locationDic {
                            list = LocationDataModel()
                            
                            if let val = item["address_id"] as? Int {
                                list.id = val
                            }
