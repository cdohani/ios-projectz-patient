...
class GetBookedProviderService: GenericService, @unchecked Sendable {
    ...
    func getData(completion: @escaping CompletionBlock, failure: @escaping FailureBlock) {
        ...
        var endPoint = String(format: Constants.URLs.getBookedProvider)
...
extension GetBookedProviderService {
    
    fileprivate func parseUserInformationFromJsonString (jsonString: String) -> FavouriteProviderReponseModel{
        
       var data = FavouriteProviderReponseModel()
            ...
            if let dictionary = try JSONSerialization.jsonObject(with: jsonString.data(using: String.Encoding(rawValue: String.Encoding.utf8.rawValue))!, options: .allowFragments) as? [String: Any] {
                ...
                if let dataDic = dictionary["data"] as? [String: Any] {
                    if let visitDic = dataDic["providers"] as? [[String: Any]] {
                        var list = FavouriteProviderModel()
                        for item in visitDic{
                            list = FavouriteProviderModel()
                            ...
                            if let val = item["provider_name"] as? String {
                                list.fullName = val
                            }
                            if let val = item["profile_image"] as? String {
                                list.providerImage = val
                            }
                            if let val = item["phone_number"] as? String {
                                list.phoneNo = val
                            }
                            if let val = item["specialities"] as? [String] {
                                list.providerSpeciality = val
                            }
                            if let val = item["appointment_count"] as? Int {
                                list.appointmentCount = val
                            }
                            if let val = item["rating"] as? Double {
                                list.rating = val
                            }
                            
                            data.arrFavouriteProvider.append(list)
                        }
                    }
