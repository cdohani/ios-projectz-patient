...
class GetAllGenderService: GenericService, @unchecked Sendable {
    ...
    func getData(completion: @escaping CompletionBlock, failure: @escaping FailureBlock) {
        ...
        var urlComponents = URLComponents(string: Constants.URLs.allGenders)
...
extension GetAllGenderService {
    
    fileprivate func parseUserInformationFromJsonString (jsonString: String) -> GenderResponseModel{
        
       var data = GenderResponseModel()
        ...
        do {
            if let dictionary = try JSONSerialization.jsonObject(with: jsonString.data(using: String.Encoding(rawValue: String.Encoding.utf8.rawValue))!, options: .allowFragments) as? [String: Any] {
                ...
                if let dataDic = dictionary["data"] as? [String: Any] {
                    if let visitDic = dataDic["genders"] as? [[String: Any]] {
                        var list = GenderInfo()
