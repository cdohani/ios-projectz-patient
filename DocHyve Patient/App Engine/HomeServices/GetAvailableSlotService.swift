...
class GetAvailableSlotService: GenericService, @unchecked Sendable {
    
    func getData(providerID : Int,addressID : Int,date : String,completion: @escaping CompletionBlock, failure: @escaping FailureBlock) {
        ...
        var endPoint = String(format: Constants.URLs.getProviderAvailableSlot, providerID)
...
extension GetAvailableSlotService {
    
    fileprivate func parseUserInformationFromJsonString (jsonString: String) -> AvailableSlotsResponseModel{
        
       var data = AvailableSlotsResponseModel()
        ...
        do {
            if let dictionary = try JSONSerialization.jsonObject(with: jsonString.data(using: String.Encoding(rawValue: String.Encoding.utf8.rawValue))!, options: .allowFragments) as? [String: Any] {
                ...
                if let dataDic = dictionary["data"] as? [String: Any] {
                    if let val = dataDic["date"] as? String  {
                        data.selectedDate = val
                    }
                    if let locationDic = dataDic["slots"] as? [[String: Any]] {
                        var list = Slots()
                        for item in locationDic {
                            list = Slots()
                            if let val = item["slot_id"] as? Int {
                                list.id = val
                            }
                            if let val = item["time"] as? String {
                                list.time = val
                            }
                            if let val = item["end_time"] as? String {
                                list.endTime = val
                            }
                            if let val = item["is_booked"] as? Bool {
                                list.isBooked = val
                            }
                            data.arrSlots.append(list)
                        }
                    }
                }
                //SessionID
