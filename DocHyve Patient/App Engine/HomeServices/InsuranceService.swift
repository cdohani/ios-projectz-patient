...
class InsuraceService: GenericService, @unchecked Sendable {
    
    func getData(type:String,completion: @escaping CompletionBlock, failure: @escaping FailureBlock) {
...
extension InsuraceService {
    
    fileprivate func parseUserInformationFromJsonString (jsonString: String) -> InsuranceReponseModel{
        
       var data = InsuranceReponseModel()
                ...
                if let dataDic = dictionary["data"] as? [String: Any] {
                    if let visitDic = dataDic["insurance_stats"] as? [[String: Any]] {
                        ...
                        for item in visitDic{
                            ...
                            if let val = item["medicare_plan"] as? Int {
                                list.medicarePlan = val
                            }
                            if let val = item["medicaid_plan"] as? Int {
                                list.medicAidPlan = val
                            }
                            if let val = item["commercial_plan"] as? Int {
                                list.commercialPlan = val
                            }
                            if let val = item["exchange_marketplace_plan"] as? Int {
                                list.exchangeMarketPlan = val
                            }
                            if let val = item["federal_tricare_program_plan"] as? Int {
                                list.federalCarePlan = val
                            }
                            if let val = item["schip_plan"] as? Int {
                                list.schipPlan = val
                            }
                            if let val = item["worker_compensation_plan"] as? Int {
                                list.workerCompensationPlan = val
                            }
