...
class GetMedicationDataService: GenericService, @unchecked Sendable {
...
extension GetMedicationDataService {
    
    fileprivate func parseUserInformationFromJsonString (jsonString: String) -> MedicationDropdownReponseModel{
        
       var data = MedicationDropdownReponseModel()
        ...
        do {
            if let dictionary = try JSONSerialization.jsonObject(with: jsonString.data(using: String.Encoding(rawValue: String.Encoding.utf8.rawValue))!, options: .allowFragments) as? [String: Any] {
                ...
                if let dataDic = dictionary["data"] as? [String: Any] {
                    ...
                    if let visitDic = dataDic["dosages"] as? [String] {
                        
                        data.arrDosage = visitDic
                    }
                    if let visitDic = dataDic["duration"] as? [String] {
                        
                        data.arrUsage = visitDic
                    }
                    if let visitDic = dataDic["frequency"] as? [String] {
                        
                        data.arrFrequency = visitDic
                    }
                }
