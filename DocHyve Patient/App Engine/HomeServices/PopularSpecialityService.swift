...
class GetPopularSpecialityService: GenericService, @unchecked Sendable {
    ...
    func getData(completion: @escaping CompletionBlock, failure: @escaping FailureBlock) {
        ...
        var urlComponents = URLComponents(string: Constants.URLs.popularSpecialities)
...
extension GetPopularSpecialityService {
    
    fileprivate func parseUserInformationFromJsonString (jsonString: String) -> PopularSpecialityReponseModel{
        
       var data = PopularSpecialityReponseModel()
        ...
        do {
            if let dictionary = try JSONSerialization.jsonObject(with: jsonString.data(using: String.Encoding(rawValue: String.Encoding.utf8.rawValue))!, options: .allowFragments) as? [String: Any] {
                ...
                if let dataDic = dictionary["data"] as? [String: Any] {
                    if let visitDic = dataDic["specialities"] as? [[String: Any]] {
                        var list = SpecialityResponseModel()
                        for item in visitDic{
                            list = SpecialityResponseModel()
                            ...
                            if let val = item["tagline"] as? String {
                                list.tagline = val
                            }
                            if let val = item["icon"] as? String {
                                list.icon = val
                            }
                            if let val = item["icon_url"] as? String {
                                list.iconUrl = val
                            }
                            if let val = item["popular"] as? Int {
                                list.isPopular = val
                            }
                            ...
                            data.arrSpeciality.append(list)
                        }
                    }
