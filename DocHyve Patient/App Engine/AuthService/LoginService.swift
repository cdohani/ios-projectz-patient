...
class LoginService: GenericService, @unchecked Sendable {
    
    func login(email:String,password:String,completion: @escaping CompletionBlock, failure: @escaping FailureBlock) {
         
         //creating payload
        let requestBodyDict  = NSMutableDictionary()
        requestBodyDict.setValue(email, forKey: "email")
        requestBodyDict.setValue(password, forKey: "password")
        requestBodyDict.setValue("patient", forKey: "user_type")
//        requestBodyDict.setValue(true, forKey: "activate_user")

        let jsonString = getJsonStringFromDictionary(requestBodyDict)
        
        let request = createURLRequest(urlString: Constants.URLs.login, requestType: .post, postData: jsonString,auth:false)
             ...
             if (error != nil) {
                 ...
                 if(jsonString!.count == 0) {
                     ...
                     if errorMessages.count > 0 {
                         ...
                         if code == 201{
                             let userData = self.parseInactiveUserFromJsonString(jsonString: jsonString ?? "")
                              completion(userData)
                         }else{
...
extension LoginService {
        ...
        do {
            if let dictionary = try JSONSerialization.jsonObject(with: jsonString.data(using: String.Encoding(rawValue: String.Encoding.utf8.rawValue))!, options: .allowFragments) as? [String: Any] {
                if let dataDic = dictionary["data"] as? [String: Any] {
                    ...
                    if let val = dataDic["status"] as? Int {
                        data.responseData.status = val
                    }
                    if let val = dataDic["message"] as? String {
                        data.responseData.message = val
                    }
                    if let userDic = dataDic["user"] as? [String: Any]  {
                        if let val = userDic["signup_step"] as? Int {
                            data.signupStep = val
                        }
                    }
                    ...
                    if let val = dataDic["signup_step"] as? Int  {
                        data.userData.signupStep = val
                        //only login user if all steps complet which is 3 otherwise on not alllowin login
                        if val == 3{
    ...
    fileprivate func parseInactiveUserFromJsonString (jsonString: String) -> LoginInActiveResponseModel {
        
       var data = LoginInActiveResponseModel()
            ...
            if let dictionary = try JSONSerialization.jsonObject(with: jsonString.data(using: String.Encoding(rawValue: String.Encoding.utf8.rawValue))!, options: .allowFragments) as? [String: Any] {
                ...
                if let dataDic = dictionary["data"] as? [String: Any] {
                    ...
                    if let userDic = dataDic["user"] as? [String: Any]  {
                        ...
                        if let val = userDic["id"] as? Int {
                            data.userInfo.id = val
                        }
                        if let val = userDic["status"] as? String {
                            data.userInfo.status = val
                        }
                        if let val = userDic["signup_step"] as? Int {
                            data.userInfo.signup_step = val
                        }
                    }
