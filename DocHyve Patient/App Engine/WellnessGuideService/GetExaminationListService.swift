...
import Foundation
class GetExaminationListService: GenericService, @unchecked Sendable {
    ...
    func getData(completion: @escaping CompletionBlock, failure: @escaping FailureBlock) {
        ...
        var endPoint = String(format: Constants.URLs.getExaminationData)
...
extension GetExaminationListService {
    
    fileprivate func parseUserInformationFromJsonString (jsonString: String) -> ExaminationResponseModel{
        
       var data = ExaminationResponseModel()
        ...
        do {
            if let dictionary = try JSONSerialization.jsonObject(with: jsonString.data(using: String.Encoding(rawValue: String.Encoding.utf8.rawValue))!, options: .allowFragments) as? [String: Any] {
                ...
                if let dataDictionary = dictionary["data"] as? [String: Any] {
                    if let examinationData = dataDictionary["examinations"] as? [[String: Any]] {
                        var list = WellnessGuideModel()
                        for item in examinationData {
                            ...
                            data.arrExamination.append(list)
                        }
                     
                    }
                    
                    if let examinationData = dataDictionary["future_date_frequencies"] as? [[String: Any]] {
                        ...
                        for item in examinationData {
                            ...
                            data.fututeFrequecy.append(list)
                        }
                    }
                    if let examinationData = dataDictionary["reminder_day_options"] as? [[String: Any]] {
                        var list = DropDownModel()
                        for item in examinationData {
                            list = DropDownModel()
                            if let val = item["id"] as? Int {
                            ...
                            data.remindeDaysrOption.append(list)
                        }
                    }
                    if let examinationData = dataDictionary["reminder_day_options"] as? [String] {
                        data.reminderDays = examinationData
                    }
                }
                //SessionID
            } else {
